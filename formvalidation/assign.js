...
function formValidate(e) {
    ...
    const phoneCheck = /(^[a-z\_\.\=\&\@]+)/; //! while i didn't include (\D) is because users might decide to start with +country-code before the actual number
    ...
    if (!phone.value) {
        setError(phone.parentElement, "error", "this input field is required");
    ...
    if (
        ...
        document.querySelector(".alert-msg").classList.add("show-alert");
        setTimeout(() => {
            document.querySelector(".alert-msg").classList.remove("show-alert");
            setBacktoDefault();
        }, 1600);
    }
}
